...
const serverDb = cloud.database();
const rawRunTransaction = serverDb.runTransaction;
serverDb.runTransaction = async (fn) => ({ result: await rawRunTransaction(fn), errMsg: 'runTransaction:ok' });
const handle = createV2Api(cloud, { database: serverDb });
let serial = 0;
